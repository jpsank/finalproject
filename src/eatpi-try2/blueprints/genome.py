...
class GenomeBP(Blueprint):
    ...
    leaf_ids: list = field(init=False)

    def __post_init__(self):
        # Leaf pins have negative keys
        self.leaf_ids = [-i - 1 for i in range(self.num_leaves)]
    ...
    def create(self, **kwargs) -> Genome:
        ...
        if "nodes" not in kwargs:
            leaves = [self.node.create(id=i) for i in self.leaf_ids]
            i = 0
            while i < len(leaves):
                leaves[i:i+2] = [self.node.create(inputs=leaves[i:i+2])]
                i += 1
            root = self.node.create()
            for i in range(self.num_leaves - 1):
                root = self.node.create(inputs=[root, self.node.create()])

            kwargs["nodes"] = {root.id: root}
            for i in self.leaf_ids:
                kwargs["nodes"][i] = self.node.create(id=i)


            nodes = {
                i: 
            }
            kwargs["nodes"] = {i: self.node.create(id=i) for i in self.leaf_ids}
