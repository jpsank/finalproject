...
> ***TASK:*** On the computer you will use to run the analyses for this exercise, create a folder called `exercise_03`, and place the files for this exercise in it. Then run `ls -l` inside this folder to make sure the files are there.
...
> ***Problem 1:*** Execute the following command to submit the job to the cluster:
...
> ***Problem 2:*** Take a look at the `seq-gen` options in the `job_mammal_gtr.sh`, the [seq-gen manual](http://htmlpreview.github.io/?https://github.com/rambaut/Seq-Gen/blob/master/documentation/Seq-Gen.Manual.html), and the model description in [Chapter 3](http://dunnlab.org/phylogenetic_biology/simulation.html#plugging-some-numbers-into-the-expanded-model). Answer the following questions about the `seq-gen` command we use.

-   What part of the command sets the nucleotide frequencies?

    **Answer**: The -f option is used to specify the equilibrium frequencies of the four nucleotides or twenty amino acids. In this case, we are simulating nucleotides (using the markov general reversible model GTR), so four decimal numbers are passed as frequencies (0.295,0.205,0.205,0.295).

-   What does `-r0.5,2,0.5,0.5,2,0.5` do?

    **Answer**: The -r option allows the user to set values for the relative rate of substitutions between nucleotide or amino acid states. Since we are simulating nucleotides, 0.5,2,0.5,0.5,2,0.5 are decimal numbers for the relative rates of substitutions from A to C, A to G, A to T, C to G, C to T and G to T respectively.

> ***Problem 3:*** Copy the job script with the following command.
...
-   What impact did making the branch lengths longer have?
    ...
    **Answer**: Making the branch lengths longer allowed more time for mutations and evolution along the tree. The 11 species' DNA sequences have more time to diverge, which can be observed in `mammal_gtr_scaled.fa`, where the sequences share less content and are more randomized than in `mammal_gtr.fa`.

## Change the model

> ***Problem 4:*** Copy the job script with the following command, and modify the settings to simulate under a JC69 model and also to change the output file name to `mammal_jc.fa`. Run the job.
